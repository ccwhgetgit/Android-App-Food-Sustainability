...
class _Login2PageState extends State<Login2Page>
  ...
  Future<FirebaseUser> _signInWithGoogle() async {
    ...
    Login2Page.user = (await _auth.signInWithCredential(credential)).user;

    DatabaseService(uid: Login2Page.user.uid).updateUserInfo(
        Login2Page.user.email, Login2Page.user.displayName, false);

    DatabaseService(uid: Login2Page.user.uid).updateUserTokens(0);
    DatabaseService(uid: Login2Page.user.uid).updateUserStatus(false);
    DatabaseService(uid: Login2Page.user.uid).updateUserPoints(0);
    DatabaseService(uid: Login2Page.user.uid).updateUserTier('Bronze');

    DatabaseService(uid: Login2Page.user.uid).updateStats();

    for (String store in StorePage.stores) {
      DatabaseService(uid: Login2Page.user.uid).resetRewards(store, 0);
    }

    Navigator.pushReplacement(context,
        MaterialPageRoute(builder: (BuildContext context) => MainInterface()));

    print("Signed in as ${Login2Page.user.displayName}");
    return Login2Page.user;
  }
  ...
  Future<FirebaseUser> _signInWithEmailPassword() async {
    FirebaseUser user;
    try {
      Login2Page.user = (await _auth.signInWithEmailAndPassword(
      ...
      DatabaseService(uid: Login2Page.user.uid)
